...
/*
	...
	udphdr_t *hdr = (udphdr_t *)((uchar *)msg + ip_pkt->ip_hdr_len*4);
	...
	printf("UDP: Data                  : \n");
	int length = hdr->len - (UDP_HEADER_SIZE + PHEADER_SIZE);
	write(STDOUT_FILENO,(hdr + UDP_HEADER_SIZE + PHEADER_SIZE),length);
*/	
}
